[CmdletBinding()]
param(
    [string]$ImageTag = "windows-dind-agent:local",
    [string]$DockerfilePath = "azsh-windows-agent/Dockerfile.windows-sh-agent-2022-windows2022-dind",
    [string]$Namespace = "default",
    [string]$PodName = "windows-dind-smoke",
    [switch]$PushImage,
    [string]$Registry,
    [switch]$KeepPod,
    ...
    [switch]$SkipEnsureWindowsDocker,
    [ValidateSet('Default', 'process', 'hyperv')]
    [string]$DockerIsolation = 'Default',
    [switch]$PlainBuildOutput,
...
$sharedInstallerPath = Join-Path $scriptRoot 'Install-DockerOnWindowsNodes.ps1'
...
function Test-Executable([string]$Name) {
    if (-not (Get-Command $Name -ErrorAction SilentlyContinue)) {
        throw "Required executable '$Name' not found in PATH."
    }
}

function ConvertFrom-SecureStringPlain([System.Security.SecureString]$SecureString) {
...
# Fallback for scenarios where the shared installer module could not be loaded (development-only).
...
# Fallback log export helper when the shared module is unavailable.
if (-not (Get-Command -Name Export-DockerInstallerLogs -CommandType Function -ErrorAction SilentlyContinue)) {
...
Test-Executable -Name 'docker'
Test-Executable -Name 'kubectl'

if (-not $AcrPassword -and $env:ACR_PASSWORD) {
...
if ([string]::IsNullOrWhiteSpace($dockerOs)) {
    throw 'Unable to determine docker engine OS type. Ensure Docker is running.'
}
if ($dockerOs -ne 'windows') {
    throw "Docker engine is using '$dockerOs' containers. Switch Docker Desktop to Windows containers before running this script."
}

$root = Resolve-Path -LiteralPath '.'
$dockerfile = Join-Path $root $DockerfilePath
if (-not (Test-Path -LiteralPath $dockerfile)) {
    throw "Dockerfile not found at $DockerfilePath"
}
$dockerfileFullPath = (Resolve-Path -LiteralPath $dockerfile).Path
$dockerfileContext = Split-Path -Path $dockerfileFullPath -Parent

$fullImage = $ImageTag
if ($Registry) {
    # Normalize short ACR name (no dots) to FQDN
    if ($Registry -notmatch '\.' -and $Registry.Length -ge 5) {
        Write-Host "Normalizing registry '$Registry' to '$Registry.azurecr.io'" -ForegroundColor Cyan
        $Registry = "$Registry.azurecr.io"
    }
}
if ($Registry) {
    if ($ImageTag -match '^[^/]+/.+') {
        Write-Verbose "Image tag already appears to contain a registry component."
    }
    else {
...
Write-Host "Building image $fullImage from $DockerfilePath" -ForegroundColor Cyan
$buildArgs = @('--pull')
$retryWithoutProgress = $false
if ($PlainBuildOutput) {
    $buildArgs += '--progress'
    $buildArgs += 'plain'
    $retryWithoutProgress = $true
}
if ($DockerIsolation -ne 'Default') {
    $buildArgs += '--isolation'
    $buildArgs += $DockerIsolation
}
$buildArgs += @('--file', $dockerfileFullPath, '-t', $fullImage, $dockerfileContext)
$buildOutput = & docker build @buildArgs 2>&1
$buildOutput | Write-Output
if ($LASTEXITCODE -ne 0 -and $retryWithoutProgress -and ($buildOutput -match 'unknown flag: --progress')) {
    Write-Warning 'docker build does not support --progress plain; retrying without the flag.'
    $buildArgs = $buildArgs | Where-Object { $_ -ne '--progress' -and $_ -ne 'plain' }
    $buildOutput = & docker build @buildArgs 2>&1
    $buildOutput | Write-Output
}
if ($LASTEXITCODE -ne 0) {
...
if ($PushImage) {
    if (-not $Registry) {
        throw 'PushImage specified but no registry provided. Use -Registry or set ImageTag to include registry prefix.'
    }

    if ($AcrUsername -and $AcrPassword) {
        Write-Host "Logging in to $Registry" -ForegroundColor Cyan
        $plainPassword = ConvertFrom-SecureStringPlain -SecureString $AcrPassword
        try {
            & docker login $Registry -u $AcrUsername -p $plainPassword | Write-Output
        }
        finally {
            $plainPassword = $null
        }
        ...
        Write-Warning 'ACR_USERNAME/ACR_PASSWORD not supplied. Assuming docker login already performed.'
    }

    Write-Host "Pushing image $fullImage" -ForegroundColor Cyan
    & docker push $fullImage | Write-Output
    if ($LASTEXITCODE -ne 0) {
        throw "docker push failed for $fullImage"
    }
}
else {
    Write-Warning "Image was not pushed. Ensure your cluster nodes can pull $fullImage (for example via local registry mirror)."
}

# Always attempt to populate ACR credentials from environment variables if not supplied explicitly
if (-not $AcrUsername -and $env:ACR_USERNAME) { $AcrUsername = $env:ACR_USERNAME }
if (-not $AcrPassword -and $env:ACR_PASSWORD) { $AcrPassword = ConvertTo-SecureString -String $env:ACR_PASSWORD -AsPlainText -Force }

# Optionally create/update imagePullSecret (independent of -PushImage)
if ($CreateImagePullSecret) {
    if (-not $ImagePullSecretName) { throw 'CreateImagePullSecret requires -ImagePullSecretName.' }
    # Derive registry host: prefer -Registry; else parse from image reference
    $targetRegistry = $Registry
    if (-not $targetRegistry) {
        # Split on first '/' if image includes registry
        $firstSegment = $fullImage.Split('/')[0]
        if ($firstSegment -match '\.') { $targetRegistry = $firstSegment }
    }
    if (-not $targetRegistry) { Write-Warning 'Cannot determine registry host for imagePullSecret creation (no -Registry and image tag has no registry prefix). Skipping.' }
    else {
        if ($AcrUsername -and $AcrPassword) {
            if ($targetRegistry -notmatch '\.azurecr\.io$') { Write-Warning "Registry '$targetRegistry' does not look like an azurecr.io host; attempting generic docker-registry secret." }
            Write-Host "Ensuring imagePullSecret '$ImagePullSecretName' for registry $targetRegistry in namespace $Namespace" -ForegroundColor Cyan
            $plainPassword = ConvertFrom-SecureStringPlain -SecureString $AcrPassword
            $secretYaml = & kubectl create secret docker-registry $ImagePullSecretName `
                --docker-server=$targetRegistry `
                --docker-username=$AcrUsername `
                --docker-password=$plainPassword `
                -n $Namespace --dry-run=client -o yaml 2>&1
            if ($LASTEXITCODE -ne 0 -or -not $secretYaml) { Write-Warning "Failed to generate secret YAML for $ImagePullSecretName (exit $LASTEXITCODE)." } else {
                $applyOut = $secretYaml | kubectl apply -f - 2>&1
                if ($LASTEXITCODE -ne 0) { Write-Warning ('kubectl apply failed for imagePullSecret {0}: {1}' -f ${ImagePullSecretName}, ($applyOut -join ' ')) } else { Write-Host "imagePullSecret $ImagePullSecretName applied/updated." }
            }
        }
        else { Write-Warning 'CreateImagePullSecret requested but ACR_USERNAME/ACR_PASSWORD not available from parameters or environment.' }
    }
}

$hostProcessLabelKey = 'kubernetes.azure.com/allow-windows-host-process-container'
$namespaceInfo = & kubectl get namespace $Namespace -o json 2>&1
if ($LASTEXITCODE -ne 0 -or -not $namespaceInfo) {
    Write-Warning ("Unable to query namespace '{0}' for hostProcess label (exit {1}). Output:`n{2}" -f $Namespace, $LASTEXITCODE, ($namespaceInfo | Out-String))
}
else {
    $needsLabel = $true
    try {
        $nsObject = ($namespaceInfo | Out-String) | ConvertFrom-Json -ErrorAction Stop
        if ($nsObject -and $nsObject.metadata -and $nsObject.metadata.labels) {
            $existingLabel = $nsObject.metadata.labels.PSObject.Properties | Where-Object { $_.Name -eq $hostProcessLabelKey }
            if ($existingLabel -and [string]::Equals([string]$existingLabel.Value, 'true', [StringComparison]::OrdinalIgnoreCase)) {
                $needsLabel = $false
            }
        }
    }
    catch {
        Write-Warning ("Failed to parse namespace JSON for '{0}': {1}" -f $Namespace, $_.Exception.Message)
    }

    if ($needsLabel) {
        Write-Host "Labeling namespace '$Namespace' with $hostProcessLabelKey=true for host-process support." -ForegroundColor Cyan
        $labelOutput = & kubectl label namespace $Namespace "$hostProcessLabelKey=true" --overwrite 2>&1
        if ($LASTEXITCODE -ne 0) {
            Write-Warning ("Failed to label namespace '{0}'. kubectl output:`n{1}" -f $Namespace, ($labelOutput | Out-String))
        }
    }
    else {
        Write-Host "Namespace '$Namespace' already labeled for host-process pods." -ForegroundColor Cyan
    }
}

$pullPolicy = if ($PushImage) { 'Always' } else { 'IfNotPresent' }
$manifestPath = Join-Path ([IO.Path]::GetTempPath()) "${PodName}-manifest.yaml"

$dockerInstallerNodes = @()

if (-not $SkipEnsureWindowsDocker) {
    Write-Host 'Ensuring Docker Engine is installed on Windows Kubernetes nodes.' -ForegroundColor Cyan
    try {
        $dockerInstallerNodes = Install-DockerOnWindowsNodes -KubeConfigPath $KubeConfigPath
    }
    catch {
        Write-Warning ("Ensure Docker step encountered an error: {0}" -f $_.Exception.Message)
    }
}
else {
    Write-Host 'SkipEnsureWindowsDocker specified; not running Docker installation helper.' -ForegroundColor Yellow
}
...
# Append imagePullSecrets lines only if requested
if ($CreateImagePullSecret -and $ImagePullSecretName) {
    $podManifestLines += '  imagePullSecrets:'
    $podManifestLines += "  - name: $ImagePullSecretName"
}
$podManifest = $podManifestLines -join "`n"

Set-Content -LiteralPath $manifestPath -Value $podManifest -Encoding ASCII

try {
    Write-Host "Deploying smoke-test pod $Namespace/$PodName" -ForegroundColor Cyan
    & kubectl apply -f $manifestPath | Write-Output

    Write-Host "Waiting for pod to become Ready" -ForegroundColor Cyan
    $waitOut = & kubectl wait --namespace $Namespace --for=condition=Ready --timeout=300s pod/$PodName 2>&1
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "Pod did not become Ready within timeout. Dumping describe & events.";
        & kubectl get pod $PodName -n $Namespace -o wide | Write-Output
        & kubectl describe pod $PodName -n $Namespace | Write-Output
        & kubectl get events -n $Namespace --sort-by=.metadata.creationTimestamp | Select-String -Pattern $PodName | Write-Output
        throw "Pod $PodName failed to become Ready. See output above for ErrImagePull or other errors."
    }
    else { $waitOut | Write-Output }

    Write-Host "Checking docker/buildx availability inside pod" -ForegroundColor Cyan
    $dockerCheckScript = @'
$ErrorActionPreference = "Stop"
$dockerExe = $null
$hintPaths = @(
    ...
    'C:\Program Files\Docker\moby-cli\docker.exe'
    'C:\Program Files\docker\docker.exe'
    'C:\Program Files\moby\docker.exe'
    'C:\Windows\System32\docker.exe'
    'C:\docker\docker.exe'
)
foreach ($candidate in $hintPaths) {
    if (Get-Command $candidate -ErrorAction SilentlyContinue) {
        $dockerExe = (Get-Command $candidate -ErrorAction Stop).Source
        break
    }
...
if (-not $dockerExe) {
    throw "docker.exe not found on host. Install Docker Engine on the Windows node or ensure its path is exposed to hostProcess pods."
}
& $dockerExe version
'@
    $dockerVersionOut = & kubectl exec -n $Namespace $PodName -- powershell -NoLogo -NoProfile -Command $dockerCheckScript
    $dockerExit = $LASTEXITCODE
    $dockerVersionOut | Write-Output
    if ($dockerExit -ne 0) {
        Write-Warning 'Docker CLI inside pod cannot reach npipe://./pipe/docker_engine. Ensure the Windows node has Docker running and hostProcess pods are permitted.'
        $testPipeCommand = 'Test-Path -Path "\\.\pipe\docker_engine"'
        & kubectl exec -n $Namespace $PodName -- powershell -NoLogo -NoProfile -Command $testPipeCommand | Write-Output
        throw "docker version failed inside pod (exit $dockerExit)."
    }

    $buildxCheckScript = @'
$ErrorActionPreference = "Stop"
if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
    $env:Path = "C:\Program Files\Docker\Docker\resources\bin;C:\Program Files\docker;" + $env:Path
}
docker buildx version
'@
    & kubectl exec -n $Namespace $PodName -- powershell -NoLogo -NoProfile -Command $buildxCheckScript | Write-Output
...
$dockerfileContent = "FROM mcr.microsoft.com/windows/nanoserver:ltsc2022`nCMD [`"cmd`",`"/c`",`"echo Windows DinD OK`"]"
Set-Content -Path (Join-Path $temp "Dockerfile") -Value $dockerfileContent -Encoding ASCII
docker build $temp -t sample-windows-dind:local
docker run --rm sample-windows-dind:local
'@

    $encoded = [Convert]::ToBase64String([Text.Encoding]::Unicode.GetBytes($buildScript))
    Write-Host "Running sample docker build inside pod" -ForegroundColor Cyan
    & kubectl exec -n $Namespace $PodName -- powershell -NoLogo -NoProfile -EncodedCommand $encoded | Write-Output
}
finally {
    ...
    if (-not $KeepPod) {
        Write-Host "Cleaning up pod $Namespace/$PodName" -ForegroundColor Cyan
        & kubectl delete pod $PodName -n $Namespace --ignore-not-found | Write-Output
    }
    else {
        Write-Host "Keeping pod $Namespace/$PodName for inspection" -ForegroundColor Yellow
    }
    try {
        if ($dockerInstallerNodes -and $dockerInstallerNodes.Count -gt 0) {
            $repoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).ProviderPath
            Export-DockerInstallerLogs -NodeNames $dockerInstallerNodes -KubeConfigPath $KubeConfigPath -RepositoryRoot $repoRoot
        }
    }
    catch {
        Write-Warning ("Failed to export Docker installer logs: {0}" -f $_.Exception.Message)
    }
}

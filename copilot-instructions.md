...
-------------------------------------------
Two temporary helpers are provided to safely simulate runs without network access or credentials. They live in `.tmp/` and are safe to run locally. They are helpful for verifying tag construction and push targets without performing real network operations. Remove them before finalizing a release PR if you prefer not to keep them in-tree.
